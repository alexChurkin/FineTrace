...
static bool IsFileExists(const char* file_name) {
  FTRACE_ASSERT(file_name != nullptr);
  FILE* file = nullptr;
#if defined(_WIN32)
...
int main(int argc, char* argv[]) {
  ...
  if (CheckBitness(GetCurrentProcess(), pinfo.hProcess)) {
    ...
    FTRACE_ASSERT(library_path_memory != nullptr);
    ...
    FTRACE_ASSERT(init_thread != nullptr);
