#ifndef FTRACE_TOOLS_COLLECTORS_CL_COLLECTOR_CL_KERNEL_COLLECTOR_H_
#define FTRACE_TOOLS_COLLECTORS_CL_COLLECTOR_CL_KERNEL_COLLECTOR_H_
...
class ClKernelCollector {
 ...
 private: // Implementation Details
  ...
  void CreateDeviceMap() {
    ...
    FTRACE_ASSERT(type == CL_DEVICE_TYPE_GPU);
  ...
  void EnableTracing(ClApiTracer* tracer) {
    ...
    FTRACE_ASSERT(enabled);
  }

  void AddKernelInstance(ClKernelInstance* instance) {
    FTRACE_ASSERT(instance != nullptr);
  ...
  void AddKernelInterval(
    ...
    if (device = device_) {
      ...
      FTRACE_ASSERT(ze_device != nullptr);
    ...
    if (device_map_.count(device) == 1 &&
      ...
      if (device_map_.count(device) == 0) { // Subdevice
        ...
        FTRACE_ASSERT(parent != nullptr);

        FTRACE_ASSERT(device_map_.count(parent) == 1);
        std::vector<cl_device_id> sub_device_list = device_map_[parent];
        FTRACE_ASSERT(!sub_device_list.empty());
        ...
        FTRACE_ASSERT(device_map_[device].empty());
        ClKernelInterval kernel_interval{
            name, device, std::vector<ClDeviceInterval>()};
        kernel_interval.device_interval_list.push_back(
  ...
  void CalculateKernelLocalSize(
    ...
    if (*(params->localWorkSize) == nullptr) {
      ...
      FTRACE_ASSERT(*(params->workDim) <= 3);
 ...
 private: // Callbacks
  ...
  static void OnEnterEnqueueKernel(
    ...
    FTRACE_ASSERT(data != nullptr);
    FTRACE_ASSERT(collector != nullptr);
    FTRACE_ASSERT(collector->device_ != nullptr);
  ...
  static void OnExitEnqueueKernel(
    ...
    if (*return_value == CL_SUCCESS) {
      ...
      FTRACE_ASSERT(*(params->event) != nullptr);
      ...
      FTRACE_ASSERT(enqueue_data != nullptr);
...
#endif // FTRACE_TOOLS_COLLECTORS_CL_COLLECTOR_CL_KERNEL_COLLECTOR_H_
