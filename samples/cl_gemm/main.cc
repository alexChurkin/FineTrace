...
static float RunAndCheck(cl_kernel kernel, cl_command_queue queue,
  ...
  FTRACE_ASSERT(kernel != nullptr && queue != nullptr);

  FTRACE_ASSERT(size > 0);
  FTRACE_ASSERT(a.size() == size * size);
  FTRACE_ASSERT(b.size() == size * size);
  FTRACE_ASSERT(c.size() == size * size);
  ...
  cl_mem dev_a = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                a.size() * sizeof(float),
                                nullptr, &status);
  FTRACE_ASSERT(status == CL_SUCCESS && dev_a != nullptr);
  cl_mem dev_b = clCreateBuffer(context, CL_MEM_READ_ONLY,
                                b.size() * sizeof(float),
                                nullptr, &status);
  FTRACE_ASSERT(status == CL_SUCCESS && dev_b != nullptr);
  cl_mem dev_c = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
                                c.size() * sizeof(float),
                                nullptr, &status);
  FTRACE_ASSERT(status == CL_SUCCESS && dev_c != nullptr);
...
static void Compute(cl_device_id device, const std::vector<float>& a,
  ...
  for (unsigned i = 0; i < repeat_count; ++i) {
    ...
    if (i == 0) { // Disable data collection for the rest iterations
      utils::SetEnv("FTRACE_ENABLE_COLLECTION", "");
    }
  }
