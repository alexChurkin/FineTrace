...
static float RunAndCheck(ze_kernel_handle_t kernel,
  ...
  FTRACE_ASSERT(status == ZE_RESULT_SUCCESS && cmd_queue != nullptr);
...
static void Compute(ze_device_handle_t device,
                    ...
                    float expected_result) {
  FTRACE_ASSERT(device != nullptr && driver != nullptr);
  FTRACE_ASSERT(size > 0 && repeat_count > 0);
  ...
  FTRACE_ASSERT(status == ZE_RESULT_SUCCESS && module != nullptr);

  ze_kernel_desc_t kernel_desc = {
      ZE_STRUCTURE_TYPE_KERNEL_DESC, nullptr, 0, "GEMM"};
  ze_kernel_handle_t kernel = nullptr;
  status = zeKernelCreate(module, &kernel_desc, &kernel);
  FTRACE_ASSERT(status == ZE_RESULT_SUCCESS && kernel != nullptr);
