#ifndef FTRACE_TOOLS_ZE_TRACER_ZE_TRACER_H_
#define FTRACE_TOOLS_ZE_TRACER_ZE_TRACER_H_
...
class ZeTracer {
 ...
 private:
  ...
  void ReportHostTiming() {
    FTRACE_ASSERT(api_collector_ != nullptr);
  ...
  void ReportDeviceTiming() {
    FTRACE_ASSERT(kernel_collector_ != nullptr);
  ...
  static void ChromeStagesCallback(
    ...
    FTRACE_ASSERT(submitted >= appended);
    stream << "{\"ph\":\"X\", \"pid\":\"" << utils::GetPid() <<
      "\", \"tid\":\"" << tid <<
      "\", \"name\":\"" << name << " (Appended)" <<
...
#endif // FTRACE_TOOLS_ZE_TRACER_ZE_TRACER_H_