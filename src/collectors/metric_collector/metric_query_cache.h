#ifndef FTRACE_TOOLS_METRIC_QUERY_CACHE_H_
#define FTRACE_TOOLS_METRIC_QUERY_CACHE_H_
 ...
 public:
  ...
  ~MetricQueryCache() {
    for (auto& value : query_map_) {
      for (auto query : value.second) {
        ...
        FTRACE_ASSERT(info != query_info_map_.end());
  ...
  zet_metric_query_handle_t GetQuery(ze_context_handle_t context) {
    ...
    if (result->second.empty()) {
      ...
      FTRACE_ASSERT(query_info_map_.count(query) == 0);
  ...
  void ReleaseQuery(zet_metric_query_handle_t query) {
    ...
    FTRACE_ASSERT(result != query_map_.end());
...
#endif // FTRACE_TOOLS_METRIC_QUERY_CACHE_H_
