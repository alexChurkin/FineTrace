#ifndef FTRACE_TOOLS_METRIC_QUERY_COLLECTOR_H_
#define FTRACE_TOOLS_METRIC_QUERY_COLLECTOR_H_
...
class MetricQueryCollector {
 ...
 private:
  ...
  void AddQuery(
    ...
    const std::lock_guard<std::mutex> lock(lock_);
    FTRACE_ASSERT(query_map_.count(command_list) == 1);
  ...
  void AddCommandList(
    ...
    FTRACE_ASSERT(query_map_.count(command_list) == 0);
    query_map_.emplace(command_list, std::vector<ZeQueryInfo>());

    FTRACE_ASSERT(context_map_.count(command_list) == 0);
  ...
  void RemoveCommandList(ze_command_list_handle_t command_list) {
    ...
    FTRACE_ASSERT(context_map_.count(command_list) == 1);
  ...
  void ProcessCommandList(ze_command_list_handle_t command_list) {
    ...
    for (auto& info : query_map_[command_list]) {
      FTRACE_ASSERT(info.event != nullptr);
 ...
 private:
  ...
  static void OnExitCommandListAppendLaunchKernel(
      ze_command_list_append_launch_kernel_params_t* params,
      ze_result_t result, void* global_data, void** instance_data) {
    FTRACE_ASSERT(instance_data != nullptr);
    zet_metric_query_handle_t query =
    ...
    if (query != nullptr) {
      ...
      ze_event_handle_t event =
        collector->event_cache_.GetEvent(
            collector->GetContext(command_list));
...
#endif // FTRACE_TOOLS_METRIC_QUERY_COLLECTOR_H_
