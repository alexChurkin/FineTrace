#ifndef FTRACE_TOOLS_METRIC_STORAGE_H_
#define FTRACE_TOOLS_METRIC_STORAGE_H_
...
class MetricStorage {
 public:
  ...
  ~MetricStorage() {
    FTRACE_ASSERT(storage_.size() == cache_.size());
  ...
  void Dump(const uint8_t* data, uint32_t size, uint32_t storage_id) {
    ...
    FTRACE_ASSERT(storage_id < storage_.size());
    FTRACE_ASSERT(storage_id < cache_.size());
    ...
    FTRACE_ASSERT(cache_[storage_id].used_size + size <=
                  cache_[storage_id].buffer.size());
    memcpy(
 ...
 private:
  MetricStorage(
    ...
    for (const auto& filename : filename_list) {
      ...
      FTRACE_ASSERT(!cache_.back().buffer.empty());
    }
  }
...
class MetricReader {
 public:
  ...
  void Read(uint32_t storage_id, size_t start, size_t size, char* data) {
    ...
    FTRACE_ASSERT(storage_[storage_id].gcount() == size);
  }
...
#endif // FTRACE_TOOLS_METRIC_STORAGE_H_
