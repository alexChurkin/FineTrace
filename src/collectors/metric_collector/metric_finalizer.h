#ifndef FTRACE_TOOLS_METRIC_FINALIZER_H_
#define FTRACE_TOOLS_METRIC_FINALIZER_H_
...
class MetricFinalizer {
 public:
  static MetricFinalizer* Create(const TraceOptions& options) {
    if (!options.CheckFlag(TRACE_RAW_METRICS) &&
        !options.CheckFlag(TRACE_KERNEL_METRICS) &&
        !options.CheckFlag(TRACE_KERNEL_INTERVALS) &&
        !options.CheckFlag(TRACE_AGGREGATION) &&
        !options.CheckFlag(TRACE_METRIC_QUERY)) {
      return nullptr;
    }
    ...
    delete reader;
    return new MetricFinalizer(data, options);
  }
  ...
  void Report() {
    ...
    if (options_.CheckFlag(TRACE_KERNEL_INTERVALS)) {
      ReportKernelIntervals();
    }

    if (options_.CheckFlag(TRACE_RAW_METRICS) ||
        options_.CheckFlag(TRACE_KERNEL_METRICS) ||
        options_.CheckFlag(TRACE_AGGREGATION)) {
    ...
    if (options_.CheckFlag(TRACE_KERNEL_METRICS) ||
        options_.CheckFlag(TRACE_AGGREGATION)) {
      cache = MakeCache();
    }
    ...
    if (options_.CheckFlag(TRACE_AGGREGATION)) {
      ReportAggregatedMetrics(cache);
    }

    if (options_.CheckFlag(TRACE_METRIC_QUERY)) {
      ReportQueryMetrics();
    }
  }

  MetricFinalizer(const MetricFinalizer& copy) = delete;
  MetricFinalizer& operator=(const MetricFinalizer& copy) = delete;

 private:
  static uint32_t GetMetricCount(zet_metric_group_handle_t group) {
    FTRACE_ASSERT(group != nullptr);
  ...
  static std::vector<std::string> GetMetricList(
    ...
    FTRACE_ASSERT(status == ZE_RESULT_SUCCESS);
    FTRACE_ASSERT(metric_count == metric_list.size());
  ...
  static size_t GetMetricId(
    ...
    FTRACE_ASSERT(!metric_list.empty());
    FTRACE_ASSERT(!metric_name.empty());
  ...
  static std::vector<zet_typed_value_t> GetMetricInterval(
    ...
    FTRACE_ASSERT(group != nullptr);
    FTRACE_ASSERT(start < end);

    size_t start_index = utils::LowerBound(cache, start);
    FTRACE_ASSERT(start_index <= cache.size());
    size_t end_index = utils::UpperBound(cache, end);
    FTRACE_ASSERT(end_index <= cache.size());
    FTRACE_ASSERT(start_index <= end_index);
  ...
  static zet_typed_value_t ComputeTotalValue(
    ...
    FTRACE_ASSERT(report_list.size() > 0);
    FTRACE_ASSERT(report_size > 0);
    FTRACE_ASSERT(metric_id < report_size);

    uint32_t report_count = report_list.size() / report_size;
    FTRACE_ASSERT(report_count * report_size == report_list.size());
    ...
    switch (report[metric_id].type) {
      case ZET_VALUE_TYPE_UINT32: {
        ...
        for (int j = 0; j < report_count; ++j) {
          ...
          total.value.ui64 += report[metric_id].value.ui32;
        }
        return total;
      }
      case ZET_VALUE_TYPE_UINT64: {
        ...
        for (int j = 0; j < report_count; ++j) {
          report = report_list.data() + j * report_size;
          total.value.ui64 += report[metric_id].value.ui64;
        }
        return total;
      }
      case ZET_VALUE_TYPE_FLOAT32: {
        ...
        for (int j = 0; j < report_count; ++j) {
          report = report_list.data() + j * report_size;
          total.value.fp64 += report[metric_id].value.fp32;
        }
        return total;
      }
      case ZET_VALUE_TYPE_FLOAT64: {
        ...
        for (int j = 0; j < report_count; ++j) {
          report = report_list.data() + j * report_size;
          total.value.fp64 += report[metric_id].value.fp64;
        }
        return total;
      }
      case ZET_VALUE_TYPE_BOOL8: {
        ...
        for (int j = 0; j < report_count; ++j) {
          report = report_list.data() + j * report_size;
          total.value.b8 |= report[metric_id].value.b8;
        }
        return total;
      }
      default:
  ...
  static zet_typed_value_t ComputeAverageValue(
    ...
    FTRACE_ASSERT(report_size > 0);
    FTRACE_ASSERT(metric_id < report_size);
    FTRACE_ASSERT(gpu_clocks_id < report_size);
    FTRACE_ASSERT(total_clocks > 0);
    ...
    switch (report[metric_id].type) {
      case ZET_VALUE_TYPE_UINT32: {
        ...
        for (int j = 0; j < report_count; ++j) {
          report = report_list.data() + j * report_size;
          total.value.ui64 +=
            report[metric_id].value.ui32 * report[gpu_clocks_id].value.ui64;
        }
      ...
      case ZET_VALUE_TYPE_UINT64: {
        ...
        for (int j = 0; j < report_count; ++j) {
          report = report_list.data() + j * report_size;
          total.value.ui64 +=
            report[metric_id].value.ui64 * report[gpu_clocks_id].value.ui64;
        }
        total.value.ui64 /= total_clocks;
        return total;
      }
      case ZET_VALUE_TYPE_FLOAT32: {
        ...
        for (int j = 0; j < report_count; ++j) {
          report = report_list.data() + j * report_size;
          total.value.fp64 +=
            report[metric_id].value.fp32 * report[gpu_clocks_id].value.ui64;
        }
      ...
      case ZET_VALUE_TYPE_FLOAT64: {
        ...
        for (int j = 0; j < report_count; ++j) {
          report = report_list.data() + j * report_size;
          total.value.fp64 +=
            report[metric_id].value.fp64 * report[gpu_clocks_id].value.ui64;
        }
        total.value.fp64 /= total_clocks;
        return total;
      }
  ...
  static std::vector<zet_typed_value_t> GetAggregatedMetrics(
    ...
    FTRACE_ASSERT(metric_list.size() == report_size);

    std::vector<zet_metric_type_t> metric_type_list = GetMetricTypeList(group);
    FTRACE_ASSERT(metric_type_list.size() == report_size);
    ...
    for (uint32_t i = 0; i < report_count; ++i) {
      ...
      FTRACE_ASSERT(report[gpu_clocks_id].type == ZET_VALUE_TYPE_UINT64);
 ...
 private:
  MetricFinalizer(ResultData* data, const TraceOptions& options)
  ...
  uint64_t ProcessReportChunk(
      ...
      uint64_t base_time) {
    FTRACE_ASSERT(!report_chunk.empty());
    FTRACE_ASSERT(group != nullptr);
    FTRACE_ASSERT(data_ != nullptr);
    ...
    FTRACE_ASSERT(report_count * report_size == report_chunk.size());
    ...
    for (uint32_t i = 0; i < report_count; ++i) {
      ...
      FTRACE_ASSERT(report[time_id].type == ZET_VALUE_TYPE_UINT64);
  ...
  bool ComputeMetrics() {
    ...
    for (size_t i = 0; i < sub_device_list.size(); ++i) {
      ...
      while (true) {
        FTRACE_ASSERT(sub_device_list.size() == metric_group_list.size());
        FTRACE_ASSERT(!metric_group_list.empty());
  ...
  std::vector< std::vector<uint64_t> > MakeCache() {
    ...
    for (size_t i = 0; i < sub_device_list.size(); ++i) {
      ...
      size_t time_id = GetMetricId(metric_list, "QueryBeginTime");
      FTRACE_ASSERT(time_id < metric_list.size());
      ...
      while (true) {
        ...
        zet_typed_value_t* report_chunk =
            reinterpret_cast<zet_typed_value_t*>(
        ...
        FTRACE_ASSERT(report_count * report_size_in_bytes == report_chunk_size);
        ...
        for (int j = 1; j < report_count; ++j) {
          ...
          FTRACE_ASSERT(report[time_id].type == ZET_VALUE_TYPE_UINT64);
          FTRACE_ASSERT(sub_device_cache.back() <= report[time_id].value.ui64);
          sub_device_cache.push_back(report[time_id].value.ui64);
        }
  ...
  void ReportKernelIntervals() {
    ...
    for (const auto& kernel_interval : data_->kernel_interval_list) {
      ...
      for (const auto& device_interval : device_interval_list) {
        FTRACE_ASSERT(
            kernel_interval.kernel_id < data_->kernel_name_list.size());
        const std::string& kernel_name =
          data_->kernel_name_list[kernel_interval.kernel_id];

        FTRACE_ASSERT(device_interval.start <= device_interval.end);
  ...
  void ReportKernelMetrics(
      const std::vector< std::vector<uint64_t> >& cache) {
    FTRACE_ASSERT(data_ != nullptr);
    ...
    for (const auto& kernel_interval : data_->kernel_interval_list) {
      ...
      for (const auto& device_interval : device_interval_list) {
        ...
        FTRACE_ASSERT(sub_device_id < sub_device_list.size());
  ...
  void ReportAggregatedMetrics(
    ...
    for (const auto& kernel_interval : data_->kernel_interval_list) {
      ...
      for (const auto& device_interval : device_interval_list) {
        ...
        FTRACE_ASSERT(report_time_id < metric_list.size());

        size_t gpu_clocks_id = GetMetricId(metric_list, "GpuCoreClocks");
        FTRACE_ASSERT(gpu_clocks_id < metric_list.size());
  ...
  void ReportQueryMetrics() {
    ...
    while (true) {
      ...
      FTRACE_ASSERT(kernel_id < data_->kernel_name_list.size());
      ...
      FTRACE_ASSERT(set_count > 0);
      std::vector<uint32_t> metric_count_list(set_count);
      FTRACE_ASSERT(total_metric_count > 0);
      ...
      for (uint32_t i = 0; i < set_count; ++i) {
        ...
        FTRACE_ASSERT(report_size == metric_list.size());
...
#endif // FTRACE_TOOLS_METRIC_FINALIZER_H_
