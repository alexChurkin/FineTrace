#ifndef FTRACE_UTILS_PROF_UTILS_H_
#define FTRACE_UTILS_PROF_UTILS_H_
...
inline cl_device_id GetClDevice(uint32_t device_id) {
  ...
  for (auto cl_dev : utils::cl::GetDeviceList(CL_DEVICE_TYPE_GPU)) {
    cl_device_pci_bus_info_khr pci_info = GetDevicePciInfo(cl_dev);
...
inline void PrintDeviceList() {
  ...
  original_state.copyfmt(std::cout);
  ...
  std::cout.copyfmt(original_state);
}

inline std::string GetMetricUnits(const char* units) {
  FTRACE_ASSERT(units != nullptr);
...
inline void PrintMetricList(uint32_t device_id) {
  ...
  FTRACE_ASSERT(device_id < device_list.size());
  ...
  for (uint32_t i = 0; i < group_count; ++i) {
    ...
    FTRACE_ASSERT(status == ZE_RESULT_SUCCESS);
    FTRACE_ASSERT(metric_count == group_props.metricCount);
...
#endif // FTRACE_UTILS_PROF_UTILS_H_
