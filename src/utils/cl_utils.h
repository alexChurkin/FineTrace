#ifndef FTRACE_UTILS_CL_UTILS_H_
#define FTRACE_UTILS_CL_UTILS_H_
...
inline std::vector<cl_device_id> CreateSubDeviceList(cl_device_id device) {
  ...
  FTRACE_ASSERT(status == CL_SUCCESS || status == CL_DEVICE_PARTITION_FAILED);
...
inline cl_device_type GetDeviceType(cl_device_id device) {
  ...
  FTRACE_ASSERT(status == CL_SUCCESS);
  FTRACE_ASSERT(type != CL_DEVICE_TYPE_ALL);
...
inline cl_queue_properties* EnableQueueProfiling(
  ...
  if (props == nullptr) {
    ...
    FTRACE_ASSERT(!(props_count&1));
    FTRACE_ASSERT(props[props_count] == 0);
...
inline size_t GetKernelSimdWidth(cl_device_id device, cl_kernel kernel) {
  FTRACE_ASSERT(device != nullptr && kernel != nullptr);
  cl_int status = CL_SUCCESS;
...
#endif // FTRACE_UTILS_CL_UTILS_H_