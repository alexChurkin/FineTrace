#ifndef FTRACE_UTILS_ZE_UTILS_H_
#define FTRACE_UTILS_ZE_UTILS_H_
...
inline ze_driver_handle_t GetGpuDriver() {
  ...
  std::string value = utils::GetEnv("FTRACE_DEVICE_ID");
  uint32_t device_id = value.empty() ? 0 : std::stoul(value);
  FTRACE_ASSERT(device_id >= 0 && device_id < driver_list.size());
...
inline ze_device_handle_t GetGpuDevice() {
  ...
  std::string value = utils::GetEnv("FTRACE_DEVICE_ID");
  uint32_t device_id = value.empty() ? 0 : std::stoul(value);
  FTRACE_ASSERT(device_id >= 0 && device_id < device_list.size());
  ...
  value = utils::GetEnv("FTRACE_SUB_DEVICE_ID");
  if (value.empty()) {
    return device_list[device_id];
  }

  uint32_t sub_device_id = value.empty() ? 0 : std::stoul(value);
  FTRACE_ASSERT(sub_device_id >= 0 && sub_device_id < sub_device_list.size());
...
inline void GetDeviceTimestamps(
  ...
  FTRACE_ASSERT(device != nullptr);
  FTRACE_ASSERT(host_timestamp != nullptr);
  FTRACE_ASSERT(device_timestamp != nullptr);
...
inline void GetMetricTimestamps(
  ...
  FTRACE_ASSERT(device != nullptr);
  FTRACE_ASSERT(host_timestamp != nullptr);
  FTRACE_ASSERT(metric_timestamp != nullptr);
  ze_result_t status = zeDeviceGetGlobalTimestamps(
...
#ifdef FTRACE_OA_TIMESTAMP_VALID_BITS
  return (1ull << FTRACE_OA_TIMESTAMP_VALID_BITS) - 1ull;
#else
  ze_device_properties_t props{ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES_1_2, };
  ze_result_t status = zeDeviceGetProperties(device, &props);
  FTRACE_ASSERT(status == ZE_RESULT_SUCCESS);
...
#endif // FTRACE_UTILS_ZE_UTILS_H_
