...
GtVregType Macro::GetVregType(size_t sizeBits) {
  FTRACE_ASSERT((64 >= sizeBits) && "Incorrect size");
...
uint64_t Macro::GetMaskBySize(size_t sizeBits) {
  FTRACE_ASSERT(sizeBits <= 64 && "Incorrect size");
  if (sizeBits == 64) {
...
GtGenProcedure Procedure::CounterInc(const IGtKernelInstrument& instrumentor,
  ...
  FTRACE_ASSERT(instrumentor.Coder().InstructionFactory().CanAccessAtomically(counterDataType) &&
...
GtGenProcedure Procedure::CounterAdd(const IGtKernelInstrument& instrumentor,
  ...
  FTRACE_ASSERT(instrumentor.Coder().InstructionFactory().CanAccessAtomically(dataGedType) &&
             "Atomic accesses with such size is not available on this HW");

  FTRACE_ASSERT(dataReg.SubRegNum() == 0 &&
...
GtGenProcedure Procedure::IsCacheLineAligned(const IGtKernelInstrument& instrumentor,
  ...
  FTRACE_ASSERT(addrRegCheckReg.IsValid() && addrRegCheckReg.ElementSize() > 0 && "Invalid register");
  FTRACE_ASSERT(simdMaskReg.IsValid() && simdMaskReg.ElementSize() > 0 && "Invalid register");
  FTRACE_ASSERT(tempData1Reg.IsValid() && tempData1Reg.ElementSize() > 0 && "Invalid register");
  FTRACE_ASSERT(flagReg.IsFlagReg() && "Resister is not a flag register");
...
GtGenProcedure Procedure::AdjustDistributionWithinBounds(const IGtKernelInstrument& instrumentor,
  ...
  FTRACE_ASSERT(bucketsNum <= mask && "Buckets number should be less than dword");
  proc += Macro::Cmp(
