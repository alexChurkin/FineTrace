...
PROF_STATUS GTPinProfiler::Start() {
  FTRACE_ASSERT(m_gtpinTool == nullptr && "Tool was not created. Error during profiler creation");
  ...
  if (m_gtpinToolHandle == nullptr) {
    ...
    FTRACE_ASSERT((m_gtpinToolHandle != nullptr) && "Tool was not registered");
  }
...
PROF_STATUS GTPinProfiler::Stop() {
  FTRACE_ASSERT(this->Status() == PROF_STATUS::ACTIVE);

  auto status = m_gtpinTool->RunWriter(m_writer);
  FTRACE_ASSERT(status == PROF_STATUS::SUCCESS);

  FTRACE_ASSERT(m_gtpinToolHandle != nullptr);
  bool result = gtpin::GTPin_GetCore()->UnregisterTool(m_gtpinToolHandle);
  if (!result) {
    std::cout << GTPIN_LAST_ERROR_STR << std::endl;
    FTRACE_ASSERT(result && "Failed to unregister tool");
  }
