...
ftrace_result ftraceElfParserCreate(const uint8_t* data, uint32_t size, elf_parser_handle_t* parser) {
  if (data == nullptr || size == 0) return FTRACE_ERROR_BAD_ARGUMENT;

  ElfParser* parser_ = new ElfParser(data, size);

  if (parser_ == nullptr) return FTRACE_ERROR_INTERNAL;
  *parser = reinterpret_cast<elf_parser_handle_t*>(parser_);

  if (!reinterpret_cast<ElfParser*>(*parser)->IsValid()) {
    delete parser_;
    return FTRACE_ERROR_INTERNAL;
  }
  return FTRACE_SUCCESS;
}

ftrace_result ftraceElfParserDestroy(elf_parser_handle_t* parser) {
  if (parser == nullptr) return FTRACE_ERROR_BAD_ARGUMENT;

  ElfParser* parser_ = reinterpret_cast<ElfParser*>(*parser);
  if (parser_->IsValid() == false) {
    return FTRACE_ERROR_BAD_ARGUMENT;
  }
  delete parser_;
  *parser = nullptr;
  return FTRACE_SUCCESS;
...
ftrace_result ftraceElfParserIsValid(elf_parser_handle_t parser, bool* is_valid) {
  if (is_valid == nullptr) return FTRACE_ERROR_BAD_ARGUMENT;
...
ftrace_result ftraceElfParserGetKernelNames(elf_parser_handle_t parser, uint32_t num_entries,
...
ftrace_result ftraceElfParserGetSourceMapping(elf_parser_handle_t parser, uint32_t kernel_index,
  ...
  if (mapping_size == 0) {
    return FTRACE_DEBUG_INFO_NOT_FOUND;
  }
...
ftrace_result ftraceElfParserGetBinaryPtr(elf_parser_handle_t parser, uint32_t kernel_index,
...
ftrace_result ElfParser::GetKernelNames(uint32_t num_entries, const char** names,
