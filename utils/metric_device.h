#ifndef FTRACE_UTILS_METRIC_DEVICE_H_
#define FTRACE_UTILS_METRIC_DEVICE_H_
...
class MetricDevice {
 public:
  static uint32_t GetDeviceCount() {
    ...
    FTRACE_ASSERT(OpenAdapterGroup != nullptr);

    md::IAdapterGroupLatest* adapter_group = nullptr;
    md::TCompletionCode status = OpenAdapterGroup(&adapter_group);
    FTRACE_ASSERT(status == md::CC_OK || status == md::CC_ALREADY_INITIALIZED);
    FTRACE_ASSERT(adapter_group != nullptr);
  ...
  static MetricDevice* Create(uint32_t device_id,
    ...
    FTRACE_ASSERT(device_id < adapter_count);
    ...
    if (sub_device_count == 0) {
      ...
      FTRACE_ASSERT(sub_device_id < sub_device_count);
  ...
  ~MetricDevice() {
    FTRACE_ASSERT(adapter_group_ != nullptr);
    FTRACE_ASSERT(adapter_ != nullptr);
    FTRACE_ASSERT(device_ != nullptr);
    FTRACE_ASSERT(lib_ != nullptr);

    md::TCompletionCode status = md::CC_OK;

    status = adapter_->CloseMetricsDevice(device_);
    FTRACE_ASSERT(status == md::CC_OK);
    status = adapter_group_->Close();
    FTRACE_ASSERT(status == md::CC_OK || status == md::CC_STILL_INITIALIZED);
  ...
  md::IConcurrentGroupLatest* FindMetricGroup(const char* set_name) {
    FTRACE_ASSERT(set_name != nullptr);
...
#endif // FTRACE_UTILS_METRIC_DEVICE_H_
